...
- `main`은 새 프로젝트에서 바로 클론해 사용할 수 있는 깨끗한 템플릿 브랜치입니다.
- `dev`는 이 템플릿의 개발 과정, 논의, 계획, 보고서, 세션 이력을 보존하는 브랜치입니다.
- 새 템플릿 작업은 기본적으로 `feat/*` 브랜치에서 만들고 `dev`에 병합합니다.
- `main`에는 `dev` 전체를 병합하지 않고, 재사용 가능한 최종 산출물만 선별 반영합니다.
- 생성 과정과 세부 이력이 필요하면 `dev` 브랜치를 확인합니다.
